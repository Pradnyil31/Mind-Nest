...
class ProfileScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<AppDocumentSnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasData && snapshot.data!.exists) {
            ...
            dailyCommitment =
                data['dailyCommitment'] as String? ?? '10 minutes';
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildSettingTile(
                        ...
                        onTap: () =>
                            _pickTime(context, user?.uid, 'bedTime', bedTime),
                      ),
                      const SizedBox(height: 12),
                      ...
                      // â”€â”€ DEVELOPER TEST SECTION â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                      _buildSectionHeader('ðŸ§ª Notification Tests'),
                      const SizedBox(height: 12),
                      _buildActionTile(
                        ...
                        onTap: () async {
                          ...
                          await ns.flutterLocalNotificationsPlugin
                              .zonedSchedule(
                                id: 99,
                          ...
                          if (context.mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                content: Text(
                                  'â± Notification will arrive in 5 seconds â€” minimize the app!',
                                ),
                            ...
                            showDialog(
                              ...
                              builder: (ctx) => AlertDialog(
                                ...
                                content: pending.isEmpty
                                    ? Text(
                                        'No notifications scheduled yet.\nSave your routine to schedule them.',
                                        style: GoogleFonts.lato(),
                                      )
                                    : SizedBox(
  ...
  Future<void> _pickTime(
    ...
    if (picked != null) {
      final formatted =
          "${picked.hour}:${picked.minute.toString().padLeft(2, '0')} ${picked.period == DayPeriod.am ? 'AM' : 'PM'}";

      // Check if we should force regenerate today's routine
      final now = DateTime.now();
      if (now.hour < 12) {
        // It's morning. Check if user has done any tasks.
        // TODO: Implement with Supabase query for routine_completions
        // For now, stubbed - assumes user completed some tasks
      }

      await FirestoreService().updateUser(uid, {'routine.$field': formatted});
      ...
      if (userData != null) {
        ...
        TimeOfDay wakeTime = _parseTime("7:00 AM");
        TimeOfDay bedTime = _parseTime("10:00 PM");
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Schedule updated! Changes will apply tomorrow.'),
            ),
          );
        }
      }
  ...
  Future<void> _pickCommitment(
    ...
    if (selected != null && selected != current) {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Commitment updated! Tasks will adjust tomorrow.'),
          ),
        );
      }
    }
          ...
          TextButton(
            onPressed: () async {
              if (controller.text.isNotEmpty) {
                ...
                if (userData != null) {
                  final motive = userData.primaryMotive ?? 'Wellness';
                  // TODO: Get routine times from user profile or routine table
                  final wakeTime = _parseTime("7:00 AM");
                  final bedTime = _parseTime("10:00 PM");
                    ...
                    onTap: () async {
                      ...
                      if (user != null) {
                        ...
                        if (hasActivityToday) {
                          ...
                          if (userData != null) {
                            final name = userData.displayName ?? 'there';
                            // TODO: Get routine times from user profile or routine table
                            final wakeTime = _parseTime("7:00 AM");
                            final bedTime = _parseTime("10:00 PM");
                          ...
                          await FirestoreService().updateUser(user.uid, {
                            ...
                            'last_generated_date': DateTime(
                              ...
                              1,
                              1,
                            ).toIso8601String(), // Force regeneration
                          });
